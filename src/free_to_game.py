...
class FreeToGame:
	...
	def get_live_games(self):
		return get(f"{self.api}/games", headers=self.headers).json()
	
	def get_games_by_platform(self, platform: str = "pc"):
		return get(
			f"{self.api}/games?platform={platform}",
			headers=self.headers).json()
	
	def get_games_by_category(self, category: str = "shooter"):
		return get(
			f"{self.api}/games?category={category}", headers=self.headers).json()
	
	def sort_games(self, sort: str):
		return get(
